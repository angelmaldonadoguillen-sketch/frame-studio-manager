...
(async()=>{
 ...
 try {
  ...
  assert.equal(await page.locator('.fp-tree-child').count(),6);
  await page.getByRole('button',{name:'Deshacer',exact:true}).click();
  assert.equal(await page.locator('.fp-tree-child').count(),5);
  ...
  // Catalog search, variants, add, delete and recovery.
  await page.locator('.fp-add-section').click();
  await page.getByLabel('Buscar módulos').fill('precios');
  await page.locator('.fp-catalog-list').getByRole('button',{name:'Precios y paquetes',exact:true}).click();
  await page.getByRole('button',{name:'Comparación',exact:true}).click();
  await page.screenshot({path:'test-results/portfolio-catalog.png'});
  ...
  await page.reload();
  assert.ok((await page.locator('.fp-tree-child').allTextContents()).includes('FORMA / Identidad'));
  ...
  assert.ok(!await page.locator('.fp-catalog').evaluate(e=>e.open));
