        ...
        {section?<>
          ...
          <div className="fp-inspector-scroll" key={(itemId||selected)+':'+tab}>
            {item?<>
              ...
              {section.type==='video'&&<FPField label="Enlace de YouTube o Vimeo"><input type="url" value={item.video||''} placeholder="https://vimeo.com/…" onChange={e=>updateItem({video:e.target.value},'video-'+itemId)}/>{item.video&&!FramePortfolio.video(item.video)&&<small className="fp-field-error">Usá un enlace original de YouTube o Vimeo.</small>}</FPField>}
              ...
              <FPField label="Enlace"><input value={item.link||''} placeholder="https://" onChange={e=>updateItem({link:e.target.value},'link-'+itemId)}/>{item.link&&!FramePortfolio.safeLink(item.link)&&<small className="fp-field-error">Usá https://, mailto: o tel:.</small>}</FPField>
            </>:tab==='design'?<>
