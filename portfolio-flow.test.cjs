...
(async()=>{
 ...
 try{
  ...
  await page.locator('.fp-inserter').getByRole('button',{name:'Video destacado',exact:true}).click();
  await page.getByRole('button',{name:'Diseño',exact:true}).click();
  await page.locator('.fp-variant-options button').filter({hasText:'Vertical'}).click();
