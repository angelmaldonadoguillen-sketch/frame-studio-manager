...
assert.match(css, /--focus-ring:\s*rgba\(245, 245, 247, 0\.72\)/);
assert.match(css, /--focus-ring:\s*rgba\(24, 24, 27, 0\.64\)/);
assert.match(css, /outline:\s*2px solid var\(--focus-ring\)/);
assert.match(css, /::selection\s*\{\s*background:\s*var\(--accent\);\s*color:\s*var\(--accent-on\)/);
...
console.log('theme-contract: 56 checks passed');
