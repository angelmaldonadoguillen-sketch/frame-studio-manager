...
(async()=>{
 ...
 try{
  ...
  await page.getByLabel('Carga de módulos').selectOption('static');
  await page.getByRole('button',{name:'Guardar borrador',exact:true}).click();
  await page.reload();await page.getByRole('button',{name:'Tema',exact:true}).click();
  assert.equal(await page.getByLabel('Carga de módulos').inputValue(),'static');
  ...
  await page.getByLabel('Carga de módulos').selectOption('progressive');
